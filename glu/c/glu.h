...
/** Static string describing the most recent error on this thread ("OK" if none). */
...
typedef enum GluTos {
    GLU_TOS_RELIABLE    = 0, /* writers block when the ring is full of unread messages */
 ...
 * NULL on failure.
 */
GluPublisher *glu_publisher_new(const char *name, uint32_t msg_size, uint32_t capacity, GluTos tos);
...
/** Convenience: memcpy `msg` into a fresh slot and commit it. Blocks while full. */
int   glu_publisher_publish(GluPublisher *pub, const void *msg);

/** Create a subscriber for topic `name`; must match msg_size/capacity of writers. */
GluSubscriber *glu_subscriber_new(const char *name, uint32_t msg_size, uint32_t capacity);
...
GluPeer *glu_peer_new(const char *name, uint32_t msg_size, uint32_t capacity, GluTos tos);
...
/** Start listening on `host:port`. Returns NULL on failure. Caller frees with glu_tcp_close_server(). */
GluTcpServer *glu_tcp_listen(uint16_t port, const char *host, const GluTcpConfig *cfg /* NULL = defaults */);
/** Same as glu_tcp_listen(port, "0.0.0.0", NULL). */
GluTcpServer *glu_tcp_listen_default(uint16_t port);

/** Block until a client connects; stores the new stream in *out_stream. Returns 0/-1. */
int  glu_tcp_accept(GluTcpServer *server, GluTcpStream **out_stream);

/** Connect to `host:port`. Returns NULL on failure. Caller frees with glu_tcp_close_stream(). */
...
int  glu_tcp_send(GluTcpStream *stream, const uint8_t *data, size_t len, size_t *sent);
/** Receive up to `cap` bytes; writes bytes actually read to *got. Returns 0/-1. */
int  glu_tcp_receive(GluTcpStream *stream, uint8_t *buf, size_t cap, size_t *got);

/** Apply `cfg` to an existing socket fd. cfg may be NULL for defaults. Returns 0 on success. */
...
GluUdpSocket *glu_udp_bind(uint16_t port, const char *host, const GluUdpSocketConfig *cfg /* NULL = defaults */);
...
int  glu_udp_send(GluUdpSocket *sock, const uint8_t *data, size_t len, size_t *sent);
...
void glu_udp_join_multicast(GluUdpSocket *sock, const char *group, uint16_t port, const char *interface);
