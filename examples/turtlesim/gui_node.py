...
class TurtlesimGUI:
    def __init__(self, root: tk.Tk):
        ...
        self.canvas = tk.Canvas(
            root, width=500, height=500, bg="#4570b5", highlightthickness=0
        )
        ...
        tk.Label(
            root,
            text="Controls: Arrow keys / WASD | Space = Stop | C = Clear",
            font=("Helvetica", 9),
            fg="#a6adc8",
            bg="#1e1e2e",
            pady=4,
    ...
    def send_cmd(self):
        lin = (2.2 if "up" in self.keys or "w" in self.keys else 0.0) - (
            2.2 if "down" in self.keys or "s" in self.keys else 0.0
        )
        ang = (2.5 if "left" in self.keys or "a" in self.keys else 0.0) - (
            2.5 if "right" in self.keys or "d" in self.keys else 0.0
        )
    ...
    def update_loop(self):
        ...
        if pose:
            ...
            self.lbl_pose.config(
                text=f"x: {pose.x:5.2f}  y: {pose.y:5.2f}  θ: {pose.theta:5.2f} rad"
            )
